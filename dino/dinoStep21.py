...
with open(spil['fil nøgle'], 'r+') as filnøgle:
	nøgle = filnøgle.read()
	#Hvis der ikke er nogen nøgle så lav en nøgle og sæt high scoren til 0
	if nøgle == "":
		filnøgle.write(str(random.randint(0,2000)))
		spil['highscore'] = 0
	#Hvis der er en nøgle så åben high score filen, læs fra den fil og decryptér det der står i den.
	else:
		with open(spil['high score fil'], 'rb') as fil:
			spil['high score'] = int(decrypt(str(nøgle), fil.read()).decode(), 2)
			fil.close()
	filnøgle.close()
...
while spil['kør']:
	...
	if spil['score'] > spil['high score']:
		...
		#Lav en ny nøgle og skriv den til nøgle filen
		with open(spil['fil nøgle'], 'w') as filnøgle:
			nøgle = random.randint(0, 2000)
			filnøgle.write(str(nøgle))
			#Åben high score filen, enkrypter den binære værdi, skriv den derefter til high score filen.
			with open(spil['high score fil'], 'wb') as fil:
				fil.write(encrypt(str(nøgle), str(bin(spil['score']))))
				fil.close()
			filnøgle.close()
	spil['score'] = 0