...
def tegnSlutskærm():
	#Hvad er den højeste klods?
	størsteTal = 0
	for række in bræt:
		if max(række) > størsteTal:
			størsteTal = max(række)
	#ÆSTETIK
	#Først sort
	pygame.draw.rect(skærm, (0,0,0), (bredde//5, højde//5, bredde//5 * 3, højde//5 * 3))
	#Så en lidt mindre en med den farve som var den højeste spilleren opnåede
	pygame.draw.rect(skærm, farver[størsteTal], (bredde//5*2, højde//5*2, bredde//5, højde//5))
	#Så skriver vi lige en besked til slut :)
	tekst = font.render("Største klods var: "+str(størsteTal), True, (255, 255, 255))
	tekstStørrelse = tekst.get_size()
	tekstX =  bredde//2 - tekstStørrelse[0]//2
	tekstY = højde//2 - tekstStørrelse[1]//2
	skærm.blit(tekst, (tekstX, tekstY))
